...
Waggle.prototype.onSocketConnect = function (socket) {
    socket.send('fields', _.pick( socket.nsp.configuration, "required", "unique" ) );

    socket.on('disconnect', this.onSocketDisconnect.bind(this, socket));
    socket.on('subscribe', this.onSocketSubscribe.bind(this, socket));
    socket.on('message', this.onSocketBroadcast.bind(this, socket));
...
Waggle.prototype.onSocketDisconnect = co(function * (socket) {
    ...
    if (sockets.length === 0) {
        ...
        socket.broadcast.send("leave", socket.unique);
 ...
 * @param {object} socket - The Socket.io instance
 */
Waggle.prototype.onSocketSubscribe = co(function * (socket, msg) {
    var scope = this;
    var room = msg.resource;
    var info = msg.info;
    ...
    _.each(uniques_in_room, co(function * (unique) {
        ...
        _.each(sockets, function(sid) {
            ...
            socket.nsp.connected[sid].send("join", stored_info);
...
Waggle.prototype.onSocketBroadcast = function ( socket, event, data ) {
    console.log(data);
};
